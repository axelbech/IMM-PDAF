...
PD = 0.9 # TODO
clutter_intensity = (np.average([z.shape[0] for z in Z]))/(700*400) # 3e-4 # TODO
gate_size = 5 # TODO
...
init_state = GaussParams(x_bar_init, P_bar_init) # tracker.init_filter_state({"mean": x_bar_init, "cov": P_bar_init})
...
for k, (Zk, x_true_k) in enumerate(zip(Z, Xgt)):
    ...
    NEES[k] = (tracker_update.mean - x_true_k[0:4]) @ la.solve(tracker_update.cov, tracker_update.mean - x_true_k[0:4]) # tracker.state_filter.NEESes(tracker_update, x_true_k) TODO
    NEESpos[k] = (tracker_update.mean[0:2] - x_true_k[0:2]) @ la.solve(tracker_update.cov[0:2,0:2], tracker_update.mean[0:2] - x_true_k[0:2]) #tracker.state_filter.NEESes(tracker_update, x_true_k, idx = [0,1]) # TODO
    NEESvel[k] = (tracker_update.mean[2:4] - x_true_k[2:4]) @ la.solve(tracker_update.cov[2:4,2:4], tracker_update.mean[2:4] - x_true_k[2:4]) #tracker.state_filter.NEESes(tracker_update, x_true_k, idx = [2,3]) # TODO
...
P_hat = np.array([upd.cov for upd in tracker_update_list]) # increases as time goes by

# calculate a performance metric
posRMSE = np.sqrt(np.mean(
    [(x[0:2]-xHat[0:2]).T@(x[0:2]-xHat[0:2]) for x, xHat in zip(Xgt, x_hat)] # NB not sure about this 'squaring'..
)) # TODO: position RMSE
velRMSE = np.sqrt(np.mean(
    [(x[2:4]-xHat[2:4]).T@(x[2:4]-xHat[2:4]) for x, xHat in zip(Xgt, x_hat)]
)) # TODO: velocity RMSE
...
CI2 = scipy.stats.chi2.interval(confprob,2) # TODO: confidence interval for NEESpos and NEESvel
CI4 = scipy.stats.chi2.interval(confprob,4) # TODO: confidence interval for NEES

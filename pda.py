...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def gate(
        ...
        *,
        ...
        M = Z.shape[0] # maybe only used if using a for loop
        g_squared = self.gate_size ** 2


        # The loop can be done using ether of these: normal loop, list comprehension or map
        gated = [self.state_filter.gate(z, filter_state, g_squared, sensor_state = sensor_state) for z in Z] # TODO: some for loop over elements of Z using self.state_filter.gate
    ...
    def loglikelihood_ratios(
        ...
        *,
        ...
        ll[1:] = [log_PD + self.state_filter.loglikelihood(z, filter_state,sensor_state=sensor_state) for z in Z]  # TODO: some for loop over elements of Z using self.state_filter.loglikelihood
    ...
    def association_probabilities(
        ...
        *,
        ...
        b = lls.max()
        ls = np.exp(lls - b) # exp-normalize, avoids overflow (normalizes after exponentiation)
    ...
    def conditional_update(
        ...
        *,
        ...
        conditional_update.append(
            filter_state # think this returns the predicted mean and covariance  TODO: missed detection 
        )
        conditional_update.extend(
            [self.state_filter.update(z, filter_state, sensor_state=sensor_state) for z in Z] # TODO: some loop over Z making a list of updates
        )
